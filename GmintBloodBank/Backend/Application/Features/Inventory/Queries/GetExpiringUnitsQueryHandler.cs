...
public sealed class GetExpiringUnitsQueryHandler : IQueryHandler<GetExpiringUnitsQuery, IReadOnlyList<ExpirationReportDto>>
{
    private readonly IRepository<BloodUnit> _bloodUnitRepository;

    public GetExpiringUnitsQueryHandler(IRepository<BloodUnit> bloodUnitRepository)
    {
        _bloodUnitRepository = bloodUnitRepository;
    }

    public async Task<IReadOnlyList<ExpirationReportDto>> HandleAsync(GetExpiringUnitsQuery query, CancellationToken cancellationToken = default)
    {
